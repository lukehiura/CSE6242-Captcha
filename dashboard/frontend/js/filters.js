...
function initFilters(points) {
  ...
  allGameTypes.forEach(gt => {
    ...
    precomputedCentroids.set(key, computeClusterCentroids(points.filter(d => d.game_type === gt)));
  });
...
function _buildFilterSvg(filterDiv, points) {
  ...
  const circleR = Math.min((fW - fm.left - fm.right) / 2, (fH - fm.top - fm.bottom) / 2 / 3) * 0.6;
  ...
  filterItems = filterSvg.selectAll(".filter-item")
    .data(GAME_FILTERS, d => d.id).join("g")
    .attr("class", "filter-item")
    .attr("transform", (d, i) => `translate(${centerX}, ${fm.top + circleR + i * circleR * 3.3})`)
    .style("cursor", "pointer");

  filterItems.append("circle")
    .attr("r", circleR)
    .attr("fill", "#888").attr("stroke", "#888").attr("stroke-width", 1.5);
  ...
  filterItems.append("g").attr("class", "icon-wrapper")
    .each(function (d) {
      ...
      g.attr("transform",
        `translate(${-bb.x * scale - bb.width * scale / 2},${-bb.y * scale - bb.height * scale / 2}) scale(${scale})`
      );
    });

  filterItems.append("text")
    .attr("class", "filters-text")
    .attr("x", 0).attr("y", circleR * 1.2)
  ...
  filterItems
    ...
    .on("click", (event, d) => {
      state.selectedGame = state.selectedGame === d.id ? null : d.id;
      updateFilterAppearance();
      applyFilter();

      if (state.selectedGame === d.id) {
        const idx = GAME_FILTERS.findIndex(f => f.id === d.id);
        const baseT = `translate(${centerX}, ${fm.top + circleR + idx * circleR * 3.3})`;
        d3.select(event.currentTarget)
          .transition().duration(80).attr("transform", `${baseT} scale(1.18)`)
          .transition().duration(180).ease(d3.easeBounceOut).attr("transform", `${baseT} scale(1)`);
      }
    });
}
...
function updateFilterAppearance() {
  ...
  filterItems.each(function (d) {
    ...
    g.select("circle")
      .attr("fill",         isSelected ? "#1a1a1a" : isHovered ? "#aaa" : "#888")
      .attr("stroke",       isSelected ? "#00d4ff" : isHovered ? "#ddd" : "#555")
      .attr("stroke-width", isSelected ? 3.5       : isHovered ? 2.5   : 1.5)
      .style("filter",      isSelected ? "drop-shadow(0 0 6px #00d4ffaa)" : "none");
    ...
    g.select(".icon-wrapper svg")
      .style("fill",    isSelected ? "#00d4ff" : isHovered ? "#fff" : "#ccc")
...
function getFilteredCentroids() {
  ...
  if (!state.selectedGame) {
    ...
    centroids = precomputedCentroids.get(key) || computeClusterCentroids(state.points);
  } else {
...
function resetAll() {
  ...
  if (typeof _trajAbortCtrl !== "undefined" && _trajAbortCtrl) {
    _trajAbortCtrl.abort(); _trajAbortCtrl = null;
  }
